...
const JuegoAldeaAmigos = () => {
  ...
  const avatarUsuario =
    Object.keys(avataresAm).find((animal) => avatarString.includes(animal)) ||
    "dragon";
  ...
  const generateProfile = async () => {
    ...
    const artistaRandom =
      artistasFavoritos[Math.floor(Math.random() * artistasFavoritos.length)];
    ...
    const ciudadRandom =
      ciudadesMX[Math.floor(Math.random() * ciudadesMX.length)];
    const juegoRandom = juegos[Math.floor(Math.random() * juegos.length)];
    const peligroRandom =
      tiposPeligrosos[Math.floor(Math.random() * tiposPeligrosos.length)];
...
Responde SOLO con este JSON:
{
      ...
      const fallbackProfiles = [
        {
          ...
          descripcion:
            "Me encanta jugar Minecraft y construir castillos. Mi personaje favorito es Steve.",
          fotos_emoji: ["🎮", "🏰", "⛏️"],
          esSeguro: true,
          razon:
            "Es un niño con intereses normales y no pide información personal.",
        ...
        },
        {
          ...
          ubicacion: "No dice",
          descripcion:
            "Hola niños, quiero ser su amigo. Les puedo dar robux gratis si me dan su contraseña.",
          fotos_emoji: ["💰", "🎁", "⚠️"],
          esSeguro: false,
          razon:
            "Es un adulto que ofrece regalos a cambio de información personal. ¡Muy peligroso!",
        ...
        },
        {
          ...
          descripcion:
            "Me gusta dibujar y jugar Roblox con mis amigas. Colecciono unicornios.",
          fotos_emoji: ["🦄", "🎨", "👑"],
          esSeguro: true,
        ...
        },
        {
          ...
          ubicacion: "No dice",
          descripcion:
            "Agregame y te envío mi WhatsApp. Puedo darte todo lo que quieras si me mandas fotos.",
          fotos_emoji: ["📱", "🚨", "⛔"],
          esSeguro: false,
          razon:
            "Pide datos de contacto y fotos. Es muy sospechoso y peligroso.",
          avatarPath: getRandomAvatar(false, "niño"),
          generadoPorIA: false,
        },
        {
          ...
          ubicacion: "Monterrey",
          descripcion:
            "Soy fan de Fortnite y me gusta hacer mapas creativos. Juego después de la escuela.",
          fotos_emoji: ["🎯", "🏗️", "🎮"],
          esSeguro: true,
        ...
        },
        {
          ...
          ubicacion: "Puebla",
          descripcion:
            "Amo a los gatos y jugar Among Us. Tengo 3 gatitos en casa.",
          fotos_emoji: ["🐱", "🎮", "🌸"],
          esSeguro: true,
        ...
        },
        {
          ...
          descripcion:
            "Te doy skins gratis de Fortnite. Solo necesito tu usuario y contraseña. También puedo darte V-Bucks.",
          fotos_emoji: ["💎", "🎁", "⚠️"],
          esSeguro: false,
        ...
        },
        {
          ...
          ubicacion: "Querétaro",
          descripcion:
            "Me gusta el fútbol y jugar FIFA. Mi equipo favorito es el Barcelona.",
          fotos_emoji: ["⚽", "🎮", "🏆"],
          esSeguro: true,
        ...
        },
        {
          ...
          ubicacion: "No dice",
          descripcion:
            "Quiero conocer niños para jugar. Puedo ser tu mentor. Hablemos en privado, no le digas a tus papás.",
          fotos_emoji: ["🎮", "🚨", "⛔"],
          esSeguro: false,
          razon:
            "Adulto que busca contacto privado con menores. Extremadamente peligroso.",
          avatarPath: getRandomAvatar(false, "niño"),
          generadoPorIA: false,
        },
        {
          ...
          ubicacion: "Tijuana",
          descripcion:
            "Me encanta dibujar anime y hacer videos de TikTok. Sueño con ser animadora.",
          fotos_emoji: ["🎨", "✨", "📱"],
          esSeguro: true,
  ...
  const iniciarJuego = async () => {
    ...
    try {
      const promises = Array(GAME_CONSTANTS.NUM_PROFILES)
        .fill()
        .map(() => generateProfile());
      ...
      const perfilesGenerados = await Promise.all(
        Array(GAME_CONSTANTS.NUM_PROFILES)
          .fill()
          .map(() => generateProfile()),
      );
      setPerfilesRestantes(perfilesGenerados.slice(1));
  ...
  const tomarDecision = (esAceptar) => {
    ...
    if (acerto) {
      ...
      setPuntosPartida(
        (prev) => prev + GAME_CONSTANTS.SCORE_PER_CORRECT_ANSWER,
      );
    } else {
      setMensaje(`❌ ¡Cuidado! ${perfilActual.razon}`);
      const tipoFallo = esAceptar
        ? "Aceptó a un perfil peligroso"
        : "Rechazó a un perfil seguro";
      setErroresPartida((prev) => [
        ...prev,
        `${tipoFallo} - Contexto: ${perfilActual.razon}`,
      ]);
    }
  ...
  const finalizarPartida = async () => {
    setPerfilActivo((prev) => ({
      ...prev,
      puntosExperiencia: (prev?.puntosExperiencia || 0) + puntosPartida,
    }));
  ...
  if (loading)
    return (
      <div style={styles.centerContainer}>
        ...
        <p style={{ marginTop: 20, fontFamily: "Poppins" }}>
          Buscando perfiles en la red...
        </p>
      </div>
    );
  ...
  return (
    <div style={{ ...styles.container, position: "relative" }}>
      ...
      <div style={{ ...styles.header, zIndex: 10 }}>
        {" "}
        {/* ---> NUEVO: zIndex para sobreponer */}
        <h1 style={{ margin: 0, fontSize: "28px" }}>🏘️ Aldea de Amigos</h1>
        <p style={{ opacity: 0.9 }}>
          Decide quién entra a tu círculo de confianza.
        </p>
      </div>
      {gameState === "playing" && perfilActual && (
        <div style={{ ...styles.cardContainer, zIndex: 10 }}>
          {" "}
          {/* ---> NUEVO: zIndex para sobreponer */}
          {perfilActual.generadoPorIA && (
            <div style={styles.iaBadge}>🤖 Generado por IA</div>
          )}
          {!perfilActual.generadoPorIA && (
            <div style={styles.fallbackBadge}>💾 Perfil de respaldo</div>
          )}
          <div style={styles.avatarContainer}>
            <img
              src={perfilActual.avatarPath}
              alt="Avatar"
          ...
          <div style={styles.infoGrid}>
            <div style={styles.infoBox}>
              <User size={16} color="#555" /> <strong>Edad:</strong>{" "}
              {perfilActual.edad}
            </div>
            <div style={styles.infoBox}>
              <MapPin size={16} color="#555" /> <strong>Ubicación:</strong>{" "}
          ...
          <div style={{ marginTop: "15px" }}>
            ...
            <div style={{ display: "flex", gap: "10px", fontSize: "24px" }}>
              {perfilActual.fotos_emoji &&
                perfilActual.fotos_emoji.map((e, i) => (
                  <span key={i}>{e}</span>
                ))}
            </div>
          </div>
      ...
      {gameState === "intro" && (
        <div style={{ ...styles.cardContainer, zIndex: 10 }}>
          ...
          <p style={{ lineHeight: 1.6 }}>
            Te llegarán solicitudes de amistad. Revisa sus fotos, descripción y
            edad.
          </p>
          <div
            style={{
          ...
          >
            <p style={{ margin: 0, fontSize: "14px" }}>
              <strong>Tip:</strong> Si algo se ve raro (piden datos, son
              adultos, o fotos extrañas), recházalo.
            </p>
          </div>
          <button style={styles.btnStart} onClick={iniciarJuego}>
            🎮 Empezar
          </button>
        </div>
      )}
      {gameState === "end" && (
        <div style={{ ...styles.cardContainer, zIndex: 10 }}>
          ...
          <p>
            Aciertos: <strong>{decisionesCorrectas}</strong> /{" "}
            {GAME_CONSTANTS.NUM_PROFILES}
          </p>
